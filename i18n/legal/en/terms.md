# Terms of Service

**Version: 2026-05-20**

---

## 1. Service description

**Norevify** is an automotive expense tracking platform for individual drivers, available at **app.norevify.com**. The service allows you to record expenses for fuel, maintenance, insurance, and other vehicle-related costs, set maintenance reminders, and track odometer readings.

---

## 2. Who can use the service

To create an account and use Norevify, you must:

- Be an **individual** (personal use only).
- Be **18 (eighteen) years of age or older**.
- Provide accurate information during registration.
- Not have a previously suspended or terminated account due to a violation of these Terms.

By creating an account, you confirm that you meet all of the above requirements.

---

## 3. Registration and account responsibilities

You are responsible for:

- **Keeping your credentials confidential**: do not share your password with third parties.
- **All actions performed with your account**: activities carried out using your access token are your responsibility.
- **Notifying us immediately** of any unauthorized access to your account by emailing **faelsabc21@gmail.com**.

We recommend using a strong password unique to Norevify.

---

## 4. Acceptable use

By using Norevify, you agree **not** to:

- Access the platform through automated means (bots, scrapers, crawlers) without prior written authorization.
- Attempt to circumvent authentication or security mechanisms.
- Interfere with the availability or integrity of the service for other users.
- Use the service for illegal purposes or in violation of any applicable law or regulation.
- Submit false data with the intent to disrupt the functioning of the system.

Misuse may result in immediate account suspension.

---

## 5. Intellectual property

The software, design, logos, text, and all other elements of Norevify are the exclusive property of their developers and are protected by applicable intellectual property laws.

These Terms do **not** transfer any intellectual property rights in the service to you. You receive only a limited, non-exclusive, non-transferable, revocable license to use the service in accordance with these Terms.

Data you enter into the platform (expenses, reminders, etc.) remains your property. You may export or delete it at any time (see Section 7).

---

## 6. Limitation of liability

Norevify is provided **"as-is"**, without express or implied warranties of uninterrupted availability, freedom from errors, or fitness for a particular purpose.

In no event shall we be liable for:

- Data loss resulting from technical failure, provided that reasonable backup practices have been implemented.
- Indirect, incidental, special, or consequential damages arising from the use or inability to use the service.
- Financial decisions made based on information displayed on the platform.

Our total liability, in any case, is limited to the amount paid by the user for the service in the 12 months preceding the event (for free-tier users: USD 0.00).

---

## 7. Account termination

**By you**: you may close your account and request deletion of all your data at any time, directly from the settings panel. After deletion, all your data is permanently removed from our servers.

**By us**: we may suspend or terminate your account, with or without notice, in the following circumstances:

- Violation of these Terms of Service.
- Use of the service for illegal or abusive purposes.
- Prolonged inactivity (per the Privacy Policy, data is deleted after 14 months without access).

In the event of termination without cause by us, we will notify the user at least 15 days in advance.

---

## 8. Changes to the service and these Terms

We may modify, suspend, or discontinue any part of the service at any time. For material changes to these Terms, we will notify active users by email at least 15 days in advance. Continued use of the service after that date constitutes acceptance of the new terms.

---

## 9. Governing law and jurisdiction

These Terms are governed by the laws of the **Federative Republic of Brazil**. For users located in the European Union or United Kingdom, applicable GDPR rights are unaffected by this choice of law. Any dispute shall be submitted to the competent courts of the user's place of domicile, in accordance with applicable consumer protection laws.

---

## 10. Contact

For questions, feedback, or requests related to these Terms:

**Email:** faelsabc21@gmail.com

---

*Last updated: 2026-05-20*
