...
module.exports = {
  ...
  findByEmailVerificationToken: (token) => User.findOne({ emailVerificationToken: token }),
  setEmailVerified: (id) =>
    User.findOneAndUpdate(
      { _id: id },
      { $set: { emailVerified: true }, $unset: { emailVerificationToken: '', emailVerificationExpiresAt: '' } },
      { returnDocument: 'after' }
    ),
  setEmailVerificationToken: (id, token, expiresAt) =>
    User.updateOne({ _id: id }, { $set: { emailVerificationToken: token, emailVerificationExpiresAt: expiresAt } }),
  setLastLoginAt: (id) =>
