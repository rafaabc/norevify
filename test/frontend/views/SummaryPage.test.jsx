...
vi.mock('@/components/charts/StackedMonthlyBar.jsx', () => ({ default: () => <svg data-testid="bar-chart" /> }));
vi.mock('@/components/charts/CategoryDonut.jsx', () => ({ default: () => <svg data-testid="donut-chart" /> }));
...
describe('SummaryPage', () => {
  ...
  it('should render summary heading', async () => {
    await act(async () => { render(<SummaryPage />); });
  ...
  it('should reload when year filter changes', async () => {
    ...
    await act(async () => {
      fireEvent.change(screen.getByLabelText(/summary\.year/), { target: { value: '2025', name: 'year' } });
    });
