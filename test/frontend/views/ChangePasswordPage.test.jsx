...
describe('ChangePasswordPage', () => {
  ...
  it('should show error when passwords do not match', async () => {
    render(<ChangePasswordPage />);
    fireEvent.change(screen.getByLabelText('auth.changePassword.new'), { target: { value: 'newpass1', name: 'newPassword' } });
    fireEvent.change(screen.getByLabelText('auth.changePassword.confirm'), { target: { value: 'newpass2', name: 'confirmPassword' } });
    await act(async () => {
      fireEvent.submit(screen.getByRole('button', { name: 'auth.changePassword.submit' }).closest('form'));
    });
  ...
  it('should call changePassword and show success on valid submit', async () => {
    render(<ChangePasswordPage />);
    fireEvent.change(screen.getByLabelText('auth.changePassword.current'), { target: { value: 'old', name: 'currentPassword' } });
    fireEvent.change(screen.getByLabelText('auth.changePassword.new'), { target: { value: 'newpass1', name: 'newPassword' } });
    fireEvent.change(screen.getByLabelText('auth.changePassword.confirm'), { target: { value: 'newpass1', name: 'confirmPassword' } });
    await act(async () => {
      fireEvent.submit(screen.getByRole('button', { name: 'auth.changePassword.submit' }).closest('form'));
    });
