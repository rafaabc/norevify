...
describe('CompleteReminderDialog', () => {
  ...
  it('should render nothing when open=false', () => {
    const { container } = render(<CompleteReminderDialog open={false} reminder={reminder} onSubmit={vi.fn()} onCancel={vi.fn()} />);
    expect(container.firstChild).toBeNull();
  });

  it('should render nothing when reminder is null', () => {
    const { container } = render(<CompleteReminderDialog open reminder={null} onSubmit={vi.fn()} onCancel={vi.fn()} />);
    expect(container.firstChild).toBeNull();
  });

  it('should render km input when open', () => {
    render(<CompleteReminderDialog open reminder={reminder} onSubmit={vi.fn()} onCancel={vi.fn()} />);
  ...
  it('should call onSubmit with parsed km on form submit', () => {
    ...
    render(<CompleteReminderDialog open reminder={reminder} onSubmit={onSubmit} onCancel={vi.fn()} />);
    fireEvent.change(screen.getByRole('spinbutton'), { target: { value: '12000' } });
  ...
  it('should call onCancel when cancel clicked', () => {
    ...
    render(<CompleteReminderDialog open reminder={reminder} onSubmit={vi.fn()} onCancel={onCancel} />);
    fireEvent.click(screen.getByText('common.cancel'));
    expect(onCancel).toHaveBeenCalledOnce();
  });

  it('should show preview when reminder has intervalKm', () => {
    render(<CompleteReminderDialog open reminder={reminderWithRecurrence} onSubmit={vi.fn()} onCancel={vi.fn()} />);
