...
// TC-03-02
test('[TC-03-02] should display expense fields correctly on edit form', async ({
  page,
  request,
}) => {
...
test('[TC-03-13] should update expense amount and redirect to expenses list', async ({
  page,
  request,
}) => {
  const expense = await createExpenseViaApi(request, token, {
...
test('[TC-03-14] should show error banner when navigating to a non-existent expense', async ({
  page,
}) => {
